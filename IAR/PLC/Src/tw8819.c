...
uint8_t InitCVBSAll[] = {
  ...
  0xFF, 0x01,   // Page 01
  0x01, 0x68,  
  0x02, 0x4C,    // for intput Y0  5? is for BW 
  0x04, 0x00,
  0x05, 0x09,
  0x06, 0x00,
  0x07, 0x12,
...
uint8_t CheckAndSetDecoderScaler( void ){
  ...
  if ( CheckDecoderVDLOSS(100) )     return( 1 );
...
uint8_t Sharpness[16] = { 
  0, 6, 13, 20, 26, 33, 40, 47, 54, 61, 67, 74, 80, 87, 94, 100 };
...
void  BlueScreenOnOff(uint8_t on){
  ...
  if ( on )    WriteTW88( REG2BF, 0x8C);      // Turn on BLUE screen with test patternt
  else
