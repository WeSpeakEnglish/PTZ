...
void P_Touch_Reset(void){
  temp = 0x02;
  HAL_I2C_Mem_Write(&hi2c2, (uint16_t)STMPE811_I2C_ADDR, (uint16_t)IOE_REG_SYS_CTRL1, I2C_MEMADD_SIZE_8BIT, &temp, 1, 20);
  DelayOnFastQ(20);
  temp = 0x00;
  HAL_I2C_Mem_Write(&hi2c2, (uint16_t)STMPE811_I2C_ADDR, (uint16_t)IOE_REG_SYS_CTRL1, I2C_MEMADD_SIZE_8BIT, &temp, 1, 20);
}
...
uint8_t P_Touch_FnctCmd(uint8_t Fct, FunctionalState NewState){
  int16_t i2c_wert;
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,(uint16_t)IOE_REG_SYS_CTRL2,I2C_MEMADD_SIZE_8BIT,&temp,1,20);
  i2c_wert = (int16_t)temp;
  ...
  HAL_I2C_Mem_Write(&hi2c2, (uint16_t)STMPE811_I2C_ADDR, (uint16_t)IOE_REG_SYS_CTRL2, I2C_MEMADD_SIZE_8BIT, &temp, 1, 20);
  return(0);
}
...
uint16_t P_Touch_ReadID(void){
  ...
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,0x00,I2C_MEMADD_SIZE_8BIT,&temp,1,20);
  i2c_wert1 = (int16_t)temp;
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,0x01,I2C_MEMADD_SIZE_8BIT,&temp,1,20);
...
uint8_t P_Touch_IOAFConfig(uint8_t IO_Pin, FunctionalState NewState){
  int16_t i2c_wert;
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,(uint16_t)IOE_REG_GPIO_AF,I2C_MEMADD_SIZE_8BIT,&temp,1,20);
  i2c_wert = (int16_t) temp;
...
uint16_t P_Touch_Read_16b(uint32_t RegisterAddr){
  ...
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,(uint16_t)RegisterAddr,I2C_MEMADD_SIZE_8BIT,&temp,1,20);
  i2c_wert1 = (int16_t) temp;
  HAL_I2C_Mem_Read(&hi2c2,(uint16_t)STMPE811_I2C_ADDR,(uint16_t)(RegisterAddr+1),I2C_MEMADD_SIZE_8BIT,&temp,1,20);
  i2c_wert2 = (int16_t) temp;
