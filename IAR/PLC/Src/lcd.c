...
void LCD_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length){
  ...
  if(hltdc.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565)  { /* RGB565 format */
...
void Fast_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t ARGB_Code){
  ...
  if(addressDraw > ProjectionLayerAddress[LayerOfView]  -1)   if(addressDraw < ProjectionLayerAddress[LayerOfView] + LAYERS_SIZE)     *(__IO uint32_t*)(ProjectionLayerAddress[LayerOfView] + 4 * (Ypos * DisplayWIDTH + Xpos)) = ARGB_Code;  //Fast, just write
}
/**
...
void DrawFastLineVertical(uint16_t x1, uint16_t y1, uint16_t y2){
  ...
  temp = 4*x1;
  while(y1 <= y2)    *(__IO uint32_t*)(address + 4 * (y1++) * DisplayWIDTH + temp) = color;
}
...
void DrawFastLineHorizontal(uint16_t y1, uint16_t x1, uint16_t x2){
  ...
  while(x1 <= x2)    *(__IO uint32_t*)(address + temp + 4*(x1++)) = color;
}
